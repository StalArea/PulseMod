package ru.openpulse.mod.utility;
...
import static ru.openpulse.mod.core.manager.client.ConfigManager.IMAGES_FOLDER;
...
public final class ThunderUtility {
    ...
    public static @NotNull String getAuthors() {
        List<String> names = PulseMod.MOD_META.getAuthors()
                .stream()
                .map(Person::getName)
                .toList();
    ...
    public static void syncVersion() {
        try {
            if (!new BufferedReader(new InputStreamReader(new URL("https://raw.githubusercontent.com/StalArea/PMUtil/main/syncVersion121.txt").openStream())).readLine().equals(PulseMod.VERSION))
                PulseMod.isOutdated = true;
    ...
    public static void parseStarGazer() {
        ...
        try {
            for (int page = 1; page <= 3; page++) {
                URL url = new URL("https://api.github.com/repos/StalArea/PulseMod/stargazers?per_page=100&page=" + page);
    ...
    public static void syncContributors() {
        try {
            URL list = new URL("https://raw.githubusercontent.com/StalArea/PMUtil/main/thTeam.txt");
            BufferedReader in = new BufferedReader(new InputStreamReader(list.openStream(), StandardCharsets.UTF_8));
            String inputLine;
            int i = 0;
            while ((inputLine = in.readLine()) != null) {
                PulseMod.contributors[i] = inputLine.trim();
                i++;
            }
            in.close();
        } catch (Exception e) {
    ...
    public static void parseCommits() {
        try {
            URL url = new URL("https://api.github.com/repos/StalArea/PulseMod/commits?per_page=25");
            BufferedReader in = new BufferedReader(new InputStreamReader(url.openStream(), StandardCharsets.UTF_8));

            changeLog.add("Changelog [Recode; Date: " + PulseMod.BUILD_DATE + "; GitHash:" + PulseMod.GITHUB_HASH + "]");
            changeLog.add("\n");
