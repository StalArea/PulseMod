...
import ru.openpulse.mod.injection.accesors.IInteractionManager;
...
import ru.openpulse.mod.utility.interfaces.IEntity;
...
import static ru.openpulse.mod.utility.player.MovementUtility.isMoving;
...
public class Speed extends Module {
    ...
    public void onSync(EventSync e) {
        ...
        if (mode.getValue() == Mode.MatrixJB) {
            ...
            if (MovementUtility.isMoving() && closeToGround && mc.player.fallDistance <= 0) {
                ...
                PulseMod.TICK_TIMER = matrixJBSpeed.getValue();
    ...
    public void modifyVelocity(EventPlayerTravel e) {
        if (mode.getValue() == Mode.GrimEntity && !e.isPre() && PulseMod.core.getSetBackTime() > 1000) {
        ...
        if ((mode.is(Mode.GrimEntity2) || mode.is(Mode.GrimCombo)) && !e.isPre() && PulseMod.core.getSetBackTime() > 1000 && MovementUtility.isMoving()) {
            int collisions = 0;
    ...
    public void onMove(EventMove event) {
        ...
        if (MovementUtility.isMoving()) {
            PulseMod.TICK_TIMER = useTimer.getValue() ? 1.088f : 1f;
