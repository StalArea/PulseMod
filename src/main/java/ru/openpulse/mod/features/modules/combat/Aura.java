...
import ru.openpulse.mod.utility.interfaces.IOtherClientPlayerEntity;
...
import static ru.openpulse.mod.utility.math.MathUtility.random;
...
public class Aura extends Module {
    ...
    public void onUpdate(PlayerUpdateEvent e) {
        ...
        if (mc.player.isUsingItem() && pauseWhileEating.getValue())
            return;
        if(pauseBaritone.getValue() && PulseMod.baritone){
    ...
    public float getAttackCooldownProgressPerTick() {
        return (float) (1.0 / mc.player.getAttributeValue(EntityAttributes.GENERIC_ATTACK_SPEED) * (20.0 * PulseMod.TICK_TIMER * (tpsSync.getValue() ? Managers.SERVER.getTPSFactor() : 1f)));
    }
