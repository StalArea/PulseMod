...
public class Timer extends Module {
    ...
    public void onUpdate() {
        switch (mode.getValue()) {
            case Normal -> PulseMod.TICK_TIMER = speed.getValue();
            case Matrix -> {
                ...
                PulseMod.TICK_TIMER = Math.max(speed.getValue(), 1f);
            ...
            case Grim -> {
                if (energy <= 0 || !isKeyPressed(boostKey) || PulseMod.core.getSetBackTime() < 2000) {
                    PulseMod.TICK_TIMER = 1f;
                    return;
                }
    ...
    public void onPacketReceive(PacketEvent.Receive e) {
        if (mode.is(Mode.Grim)) {
            if (e.getPacket() instanceof CommonPingS2CPacket
                    && PulseMod.core.getSetBackTime() > 2000) {
