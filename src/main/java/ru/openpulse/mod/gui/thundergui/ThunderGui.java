package ru.openpulse.mod.gui.thundergui;
...
import ru.openpulse.mod.features.modules.client.ThunderHackGui;
import ru.openpulse.mod.gui.font.FontRenderers;
import ru.openpulse.mod.gui.thundergui.components.*;
...
import ru.openpulse.mod.utility.render.animation.EaseOutBack;
...
public class ThunderGui extends Screen {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int clickedButton) {
        ...
        if (isHoveringItem(main_posX + 250, main_posY + 15, 140, 10, (float) mouseX, (float) mouseY) && currentMode == CurrentMode.Modules) {
            searching = true;
            PulseMod.currentKeyListener = PulseMod.KeyListening.ThunderGui;
        }
    ...
    public void keyTyped(String typedChar, int keyCode) throws IOException {
        if (PulseMod.currentKeyListener != PulseMod.KeyListening.Sliders && PulseMod.currentKeyListener != PulseMod.KeyListening.ThunderGui)
            return;
