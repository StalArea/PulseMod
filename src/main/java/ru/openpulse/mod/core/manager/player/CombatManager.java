package ru.openpulse.mod.core.manager.player;
...
import ru.openpulse.mod.events.impl.TotemPopEvent;
import ru.openpulse.mod.features.modules.Module;
import ru.openpulse.mod.features.modules.combat.AntiBot;
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (event.getPacket() instanceof EntityStatusS2CPacket pac) {
            if (pac.getStatus() == EntityStatuses.USE_TOTEM_OF_UNDYING) {
                ...
                PulseMod.EVENT_BUS.post(new TotemPopEvent((PlayerEntity) ent, popList.get(ent.getName().getString())));
            }
        }
    }
